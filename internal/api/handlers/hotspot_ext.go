...
func (h *HotspotExtHandler) DeleteExpired(c fiber.Ctx) error {
	...
	return c.JSON(fiber.Map{"success": true, "deleted": result.RowsAffected, "count": result.RowsAffected})
}
...
func (h *HotspotExtHandler) DeleteMultiple(c fiber.Ctx) error {
	...
	// Accept either "ids" or "voucherIds"
	ids := body.IDs
	if len(ids) == 0 {
		ids = body.VoucherIDs
	}
	...
	result := h.db.Where("id IN ?", ids).Delete(&models.HotspotVoucher{})
	return c.JSON(fiber.Map{"success": true, "deleted": result.RowsAffected, "count": result.RowsAffected})
}
