...
func (p *Poller) poll(ctx context.Context, olt *models.NetworkOLT, pool *telnet.Pool) {
	...
	// We now pass nil for ponPorts so it automatically scans all 16 ports on both boards
	// instead of being restricted to only the ports currently known in the database.
	result := zte.DiscoverAll(ctx, snmpCfg, pool, nil)
