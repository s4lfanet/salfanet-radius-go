...
<div class="mb-6">
    <div class="flex items-center space-x-3">
        ...
        <h1 class="text-2xl font-bold" style="color: var(--text-primary);">Edit Script Template</h1>
    </div>
</div>
    ...
    <form action="{{ route('script-templates.update', $scriptTemplate) }}" method="POST" class="p-6">
        ...
        <div class="space-y-5">
            <div>
                ...
                <input type="text" name="merk" id="merk" value="{{ old('merk', $scriptTemplate->merk) }}" required class="block w-full rounded-xl sm:text-sm px-4 py-2.5 transition-all duration-200 focus:outline-none focus:ring-2" style="background: var(--bg-primary); border: 1px solid var(--border-color); color: var(--text-primary); --tw-ring-color: var(--accent);">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <textarea name="pon_onu_mng_script" id="pon_onu_mng_script" rows="14" class="w-full p-4 text-xs font-mono rounded-xl whitespace-pre transition-all duration-200 focus:outline-none focus:ring-2" style="background: var(--bg-primary); border: 1px solid var(--border-color); color: var(--accent); --tw-ring-color: var(--accent); line-height: 1.6;" spellcheck="false" required>{{ old('pon_onu_mng_script', $scriptTemplate->pon_onu_mng_script) }}</textarea>
                    @error('pon_onu_mng_script') <p class="mt-1 text-sm" style="color: #f87171;">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="flex items-center pt-2">
                <input id="is_default" name="is_default" type="checkbox" value="1" {{ old('is_default', $scriptTemplate->is_default) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-600 focus:ring-cyan-500" style="background: var(--bg-primary); color: var(--accent);">
        ...
        <div class="mt-8 flex justify-end space-x-3">
            ...
            <button type="submit" class="py-2.5 px-5 rounded-xl text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:shadow-cyan-500/25 hover:scale-105" style="background: linear-gradient(135deg, #06b6d4, #0891b2);">Update Template</button>
        </div>
    </form>
</div>
@endsection
