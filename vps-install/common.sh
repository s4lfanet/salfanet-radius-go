...
# Generate random passwords if not provided (security: no hardcoded defaults)
export DB_PASSWORD="${DB_PASSWORD:-$(head -c 16 /dev/urandom | base64 | tr -dc 'A-Za-z0-9' | head -c 20)}"
export DB_ROOT_PASSWORD="${DB_ROOT_PASSWORD:-$(head -c 16 /dev/urandom | base64 | tr -dc 'A-Za-z0-9' | head -c 24)}"
