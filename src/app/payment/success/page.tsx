...
function PaymentSuccessContent() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-secondary relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="text-center relative z-10">
        <Loader2 className="w-10 h-10 animate-spin text-[success] mx-auto mb-3 drop-shadow-[0_0_20px_rgba(18, 183, 106,0.6)]" />
  ...
  if (error || (!invoice && !deposit)) return (
    <div className="min-h-screen bg-secondary relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="relative z-10 max-w-sm w-full bg-secondary/80 backdrop-blur-xl rounded-xl border-2 border-[#ff4466]/50 p-6 text-center shadow-[0_0_40px_rgba(255,68,102,0.2)]">
        ...
        <button onClick={() => router.push('/')} className="px-6 py-2.5 text-xs font-bold bg-gradient-to-r from-brand-600 to-brand-400 text-white rounded-xl shadow-[0_0_20px_rgba(122, 90, 248,0.3)]">{t('common.back')}</button>
      </div>
    </div>
  );
        ...
        <div className="max-w-md mx-auto space-y-4 relative z-10">
          <div className="text-center">
            <div className="inline-block relative mb-4">
              ...
              <div className="relative w-20 h-20 bg-gradient-to-br from-[success] to-brand-400 rounded-full flex items-center justify-center mx-auto shadow-[0_0_40px_rgba(18, 183, 106,0.5)]">
                <CheckCircle2 className="w-10 h-10 text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]" />
              </div>
            </div>
            <h1 className="text-xl font-bold text-[success] mb-1">{t('payment.depositSuccess')}</h1>
            <p className="text-xs text-muted-foreground/70">{isPaid ? t('payment.balanceAdded') : t('payment.processing')}</p>
          </div>

          <div className="bg-secondary/80 backdrop-blur-xl rounded-xl border-2 border-[success]/50 overflow-hidden shadow-[0_0_40px_rgba(18, 183, 106,0.2)]">
            <div className="bg-gradient-to-r from-[success] to-brand-400 px-4 py-3">
            ...
            <div className="p-5 space-y-4">
              <div className="text-center py-5 bg-gradient-to-br from-[success]/10 to-brand-400/10 rounded-xl border border-[success]/20">
                <p className="text-[10px] text-muted-foreground/60 mb-1">{t('payment.depositAmount')}</p>
                <p className="text-3xl font-bold text-[success] drop-shadow-[0_0_15px_rgba(18, 183, 106,0.5)]">{formatCurrency(deposit.amount)}</p>
              </div>
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 bg-input/50 rounded-xl">
                ...
                {isPaid && (
                  <div className="flex items-start gap-3 p-3 bg-input/50 rounded-xl">
                    <Wallet className="w-4 h-4 text-[success] mt-0.5" />
                    <div>
                      <p className="text-[10px] text-muted-foreground/60">{t('payment.currentBalanceNow')}</p>
                      <p className="text-xs font-bold text-[success]">{formatCurrency(deposit.newBalance)}</p>
                    </div>
                  </div>
                )}
              </div>
              {isPaid && (
                <div className="bg-[success]/10 border border-[success]/30 rounded-xl p-3">
                  <p className="text-xs text-[success] text-center font-medium">[OK]  {t('payment.balanceAddedToAccount')}</p>
                </div>
              )}
            </div>
          </div>

          <button onClick={() => router.push('/agent/dashboard')} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold text-white bg-gradient-to-r from-brand-600 to-brand-400 rounded-xl shadow-[0_0_25px_rgba(122, 90, 248,0.4)]">
    ...
    <div className="min-h-screen bg-secondary relative py-6 px-4">
      ...
      <div className="max-w-md mx-auto space-y-4 relative z-10">
        <div className="text-center">
          ...
          <h1 className="text-xl font-bold text-[success] mb-1">{t('payment.paymentSuccess')}</h1>
        ...
        <div className="bg-secondary/80 backdrop-blur-xl rounded-xl border-2 border-[success]/50 overflow-hidden shadow-[0_0_40px_rgba(18, 183, 106,0.2)]">
          ...
          <div className="p-5 space-y-4">
            <div className="text-center py-5 bg-gradient-to-br from-[success]/10 to-brand-400/10 rounded-xl border border-[success]/20">
              ...
              <p className="text-3xl font-bold text-[success] drop-shadow-[0_0_15px_rgba(18, 183, 106,0.5)]">{formatCurrency(invoice?.amount || 0)}</p>
            </div>
            <div className="space-y-3">
              <div className="flex items-start gap-3 p-3 bg-input/50 rounded-xl">
                <User className="w-4 h-4 text-brand-600 mt-0.5" />
              ...
              {expiryDate && (
                <div className="flex items-start gap-3 p-3 bg-input/50 rounded-xl">
                  <Calendar className="w-4 h-4 text-accent-foreground mt-0.5" />
                  <div>
                    <p className="text-[10px] text-muted-foreground/60">{t('payment.activeUntil')}</p>
                    <p className="text-xs font-bold text-[success]">{formatWIB(expiryDate, 'd MMMM yyyy')}</p>
                  </div>
                </div>
              )}
            </div>
            {isPaid && (
              <div className="bg-[success]/10 border border-[success]/30 rounded-xl p-3">
                <p className="text-xs text-[success] text-center font-medium">[OK]  {t('payment.serviceActivated')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={() => router.push(`/pay/${token}`)} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 text-xs font-medium bg-input text-white rounded-xl border-2 border-brand-600/30 hover:border-[success]">
            <Download className="w-4 h-4 text-brand-400" />{t('payment.viewInvoice')}
          </button>
          <button onClick={() => router.push('/')} className="flex-1 flex items-center justify-center gap-2 px-4 py-3 text-xs font-bold text-white bg-gradient-to-r from-[success] to-brand-400 rounded-xl shadow-[0_0_20px_rgba(18, 183, 106,0.3)]">
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-secondary flex items-center justify-center">
        <Loader2 className="w-10 h-10 animate-spin text-[success] drop-shadow-[0_0_20px_rgba(18, 183, 106,0.6)]" />
      </div>
    }>
