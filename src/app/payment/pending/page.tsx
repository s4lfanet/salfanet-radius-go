  ...
  if (loading) return (
    <div className="min-h-screen bg-secondary relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="text-center relative z-10">
        ...
        <p className="text-xs text-muted-foreground/70">Mengecek status...</p>
      </div>
    </div>
  );
  ...
  if (error || !invoice) return (
    <div className="min-h-screen bg-secondary relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="relative z-10 max-w-sm w-full bg-secondary/80 backdrop-blur-xl rounded-xl border-2 border-[#ff4466]/50 p-6 text-center shadow-[0_0_40px_rgba(255,68,102,0.2)]">
        ...
        <button onClick={() => router.push('/')} className="px-6 py-2.5 text-xs font-bold bg-gradient-to-r from-brand-600 to-brand-400 text-white rounded-xl">Kembali</button>
      </div>
    </div>
  );
      ...
      <div className="max-w-md mx-auto space-y-4 relative z-10">
        <div className="text-center">
          <div className="inline-block relative mb-4">
            <div className="absolute inset-0 bg-accent/30 rounded-full animate-pulse"></div>
            <div className="relative w-20 h-20 bg-gradient-to-br from-accent-foreground to-brand-600 rounded-full flex items-center justify-center mx-auto shadow-[0_0_40px_rgba(238, 70, 188,0.5)]">
              <Clock className="w-10 h-10 text-white animate-bounce drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]" />
            </div>
          </div>
          <h1 className="text-xl font-bold text-accent-foreground mb-1">Menunggu Pembayaran</h1>
          <p className="text-xs text-muted-foreground/70">Pembayaran sedang diproses</p>
        </div>

        <div className="bg-secondary/80 backdrop-blur-xl rounded-xl border-2 border-accent/50 overflow-hidden shadow-[0_0_40px_rgba(238, 70, 188,0.2)]">
          <div className="bg-gradient-to-r from-accent-foreground to-brand-600 px-4 py-3">
          ...
          <div className="p-5 space-y-4">
            <div className="text-center py-5 bg-gradient-to-br from-accent-foreground/10 to-brand-600/10 rounded-xl border border-accent/20">
              <p className="text-[10px] text-muted-foreground/60 mb-1">Total Pembayaran</p>
            ...
            <div className="bg-accent/10 border border-accent/30 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="text-xs text-muted-foreground/80 space-y-1.5">
                  <p className="font-bold text-accent-foreground">Langkah Selanjutnya:</p>
            ...
            <div className="flex items-center justify-center gap-2 text-[10px] text-muted-foreground/60">
              <div className="w-2 h-2 bg-[success] rounded-full animate-pulse shadow-[0_0_10px_rgba(18, 183, 106,0.5)]"></div>
              <span>Auto-refresh setiap 5 detik</span>
            </div>

            {invoice.paymentLink && (
              <a href={invoice.paymentLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full px-4 py-3 text-sm font-bold text-white bg-gradient-to-r from-brand-600 to-brand-400 rounded-xl shadow-[0_0_25px_rgba(122, 90, 248,0.4)] hover:shadow-[0_0_35px_rgba(122, 90, 248,0.6)] transition-all">
        ...
        <div className="space-y-2">
          <button onClick={() => { setChecking(true); fetchInvoiceStatus(); }} disabled={checking} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-xs font-medium bg-input text-white rounded-xl border-2 border-brand-600/30 hover:border-brand-500 disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 text-brand-400 ${checking ? 'animate-spin' : ''}`} />{checking ? 'Mengecek...' : 'Cek Status'}
          </button>
          <button
            onClick={() => {
            ...
            className="w-full px-4 py-3 text-xs font-medium text-muted-foreground/70 hover:text-white"
          >
            Kembali ke Invoice
          </button>
        </div>

        <p className="text-center text-[10px] text-muted-foreground/50">Halaman akan otomatis redirect setelah pembayaran berhasil</p>
      </div>
    </div>
  );
}
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-secondary flex items-center justify-center">
        <Loader2 className="w-10 h-10 animate-spin text-brand-400 drop-shadow-[0_0_20px_rgba(70, 95, 255,0.6)]" />
      </div>
    }>
