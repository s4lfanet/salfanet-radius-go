      ...
      if (response.ok) {
        ...
        if (data) {
          setSettings({
            ...
            logo: data.logo || '',
            qrisStaticCode: data.qrisStaticCode || '',
            ...
            <div className="space-y-3">
              ...
              <div className="border border-border rounded-lg p-3 bg-card/50">
                ...
                <p className="text-[10px] text-muted-foreground mb-3">
                  Gunakan QRIS dari rekening bank Anda sendiri tanpa biaya admin payment gateway. Sistem akan otomatis konversi QRIS statis menjadi dinamis dengan nominal tagihan pelanggan.
                </p>

                {settings.qrisEnabled && (
                  <div className="space-y-3">
                    <div>
                      <label className="flex items-center gap-1.5 text-[10px] font-medium text-foreground mb-1">
                        Kode QRIS Statis
                      </label>
                      <textarea
                        value={settings.qrisStaticCode}
                        onChange={(e) => setSettings({ ...settings, qrisStaticCode: e.target.value.trim() })}
                        className="w-full px-2.5 py-1.5 text-xs font-mono border border-border rounded-lg bg-card focus:ring-1 focus:ring-ring focus:border-primary"
                        rows={3}
                    ...
                    <div>
                      <label className="flex items-center gap-1.5 text-[10px] font-medium text-foreground mb-1">
                        Nama Merchant (tampil di halaman bayar)
                      </label>
                      <input
                        type="text"
