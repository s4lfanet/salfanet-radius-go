  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        ...
        <RefreshCcw className="h-12 w-12 animate-spin text-brand-500 dark:text-brand-400 dark:drop-shadow-[0_0_20px_rgba(70, 95, 255,0.6)] relative z-10" />
      </div>
    );
  }
    ...
    <div className="bg-background relative">
      ...
      <div className="relative z-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-400 dark:via-white dark:to-accent-foreground dark:drop-shadow-[0_0_30px_rgba(70, 95, 255,0.5)] flex items-center gap-3">
              <Link2 className="h-6 w-6 text-brand-400 drop-shadow-[0_0_15px_rgba(70, 95, 255,0.6)]" />
          ...
          <button
            ...
            className="inline-flex items-center px-4 py-2.5 text-sm font-bold bg-brand-500 text-black rounded-lg hover:bg-brand-500/90 transition-all shadow-[0_0_20px_rgba(70, 95, 255,0.4)] uppercase tracking-wide"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-brand-400 uppercase tracking-wide">{t('network.totalAssignments')}</p>
                <p className="text-lg sm:text-2xl font-bold text-foreground mt-1">{assignments.length}</p>
              </div>
              <Link2 className="h-8 w-8 text-brand-400 drop-shadow-[0_0_15px_rgba(70, 95, 255,0.6)]" />
            </div>
          </div>
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-brand-400 uppercase tracking-wide">{t('network.uniqueOdps')}</p>
                <p className="text-lg sm:text-2xl font-bold text-foreground mt-1">
                  {new Set(assignments.map(a => a.odpId)).size}
                </p>
              </div>
              <Box className="h-8 w-8 text-brand-400 drop-shadow-[0_0_15px_rgba(70, 95, 255,0.6)]" />
            </div>
          </div>
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)]">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-brand-400 uppercase tracking-wide">{t('network.uniqueCustomers')}</p>
                <p className="text-lg sm:text-2xl font-bold text-foreground mt-1">
                  {new Set(assignments.map(a => a.customerId)).size}
                </p>
              </div>
              <Users className="h-8 w-8 text-brand-400 drop-shadow-[0_0_15px_rgba(70, 95, 255,0.6)]" />
            </div>
          </div>
        </div>
        ...
        <div className="block md:hidden space-y-3">
          {filteredAssignments.length === 0 ? (
            <div className="bg-card/80 backdrop-blur-xl rounded-xl border border-brand-600/20 p-4 text-center text-muted-foreground text-xs">
              {t('network.noAssignmentsFound')}
            </div>
          ) : (
            filteredAssignments.map((assignment) => (
              <div key={assignment.id} className="bg-card/80 backdrop-blur-xl rounded-xl border border-brand-600/20 p-3">
          ...
          <form onSubmit={handleSubmit}>
            <ModalBody className="space-y-4">
              ...
              <div>
                ...
                {selectedCustomer ? (
                  <div className="p-2 bg-brand-500/10 border border-brand-500/30 rounded-lg">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                  ...
                  <div className="relative">
                    <input type="text" value={customerSearch} onChange={(e) => { setCustomerSearch(e.target.value); searchCustomers(e.target.value); }} placeholder={t('network.searchCustomerPlaceholder')} className="w-full px-3 py-2 text-xs bg-background dark:bg-input border border-brand-600/40 rounded-lg text-foreground placeholder:text-muted-foreground/40 focus:border-brand-500 focus:ring-1 focus:ring-[brand-400]/30 transition-all" />
                    {isSearching && (<RefreshCcw className="absolute right-2 top-1/2 -translate-y-1/2 h-3 w-3 animate-spin text-brand-400" />)}
                    {searchResults.length > 0 && (
                      <div className="absolute z-10 w-full mt-1 bg-popover dark:bg-input border border-brand-600/50 rounded-lg shadow-lg max-h-48 overflow-y-auto">
              ...
              {selectedCustomer && (
                <div>
                  <ModalLabel required>{t('network.selectOdp')} {loadingNearestOdps && (<RefreshCcw className="inline h-2.5 w-2.5 ml-1 animate-spin text-brand-400" />)}</ModalLabel>
                  {nearestOdps.length === 0 && !loadingNearestOdps ? (
                    <div className="p-3 bg-muted/50 dark:bg-input/50 border border-brand-600/30 rounded-lg text-xs text-muted-foreground text-center">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto">
                      {nearestOdps.map(odp => (
                        <button key={odp.id} type="button" onClick={() => handleOdpSelect(odp)} disabled={(odp.availablePorts?.length || 0) === 0} className={`p-2 text-left rounded-lg border transition-all ${selectedOdpId === odp.id ? 'bg-brand-500/20 border-brand-500 shadow-[0_0_10px_rgba(70, 95, 255,0.3)]' : (odp.availablePorts?.length || 0) === 0 ? 'bg-muted/50 dark:bg-input/50 border-brand-600/20 opacity-50 cursor-not-allowed' : 'border-brand-600/30 hover:border-brand-500/50'}`}>
                          <div className="flex items-center gap-1">
              ...
              {selectedOdpId && (
                <div>
                  ...
                  <ModalSelect value={selectedPort} onChange={(e) => setSelectedPort(e.target.value)} required>
                    <option value="" className="dark:bg-input">{t('network.selectPort')}</option>
                    {(nearestOdps.find(o => o.id === selectedOdpId)?.availablePorts || []).map(port => (<option key={port} value={port} className="dark:bg-input">{t('network.port')} {port}</option>))}
                  </ModalSelect>
                </div>
              )}
