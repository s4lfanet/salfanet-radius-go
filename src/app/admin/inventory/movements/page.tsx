          ...
          <div className="block md:hidden space-y-3">
            {filteredMovements.length === 0 ? (
              ...
              filteredMovements.map((movement) => (
                <div key={movement.id} className="bg-card/80 backdrop-blur-xl rounded-xl border border-brand-600/20 p-3">
              ...
              <ModalBody className="space-y-4">
                <div>
                  ...
                  <ModalSelect value={formData.itemId} onChange={(e) => setFormData({ ...formData, itemId: e.target.value })} required>
                    <option value="" className="dark:bg-input">Select Item</option>
                    {items.map((item) => (<option key={item.id} value={item.id} className="dark:bg-input">{item.name} ({item.sku})</option>))}
                  </ModalSelect>
                </div>
                <div>
                  ...
                  <ModalSelect value={formData.movementType} onChange={(e) => setFormData({ ...formData, movementType: e.target.value })} required>
                    <option value="IN" className="dark:bg-input">Stock In (Purchase/Return)</option>
                    <option value="OUT" className="dark:bg-input">Stock Out (Sale/Usage)</option>
                    <option value="ADJUSTMENT" className="dark:bg-input">Adjustment (Correction)</option>
                  </ModalSelect>
                </div>
                <div>
