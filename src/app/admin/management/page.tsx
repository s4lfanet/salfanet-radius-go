    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        ...
        <div className="flex flex-col items-center gap-2 relative z-10">
          <div className="w-12 h-12 border-2 border-brand-500 dark:border-brand-500 border-t-transparent rounded-full animate-spin dark:drop-shadow-[0_0_20px_rgba(70, 95, 255,0.6)]" />
      ...
      <div className="max-w-7xl mx-auto relative z-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-400 dark:via-white dark:to-accent-foreground dark:drop-shadow-[0_0_30px_rgba(70, 95, 255,0.5)]">{t('management.title')}</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-3 shadow-[0_0_20px_rgba(122, 90, 248,0.2)] hover:border-brand-600/50 transition-all">
            ...
            <ModalBody className="space-y-4">
              ...
              <div>
                ...
                <ModalSelect value={formData.role} onChange={(e) => handleRoleChange(e.target.value)}>
                  {ROLES.filter((role) => role.value !== 'SUPER_ADMIN' || currentUserIsSuperAdmin).map((role) => (<option key={role.value} value={role.value} className="dark:bg-input">{t(`management.${role.translationKey}`)}</option>))}
                </ModalSelect>
              ...
              <div>
                ...
                <div className="border border-brand-600/30 rounded-lg p-2 max-h-48 overflow-y-auto space-y-2 bg-muted/50 dark:bg-input/50">
                  {Object.entries(permissionsByCategory).map(([category, perms]) => (
                    <div key={category}>
                      <p className="text-[10px] font-semibold text-brand-400 mb-1 uppercase tracking-wider">{category}</p>
                      <div className="grid grid-cols-2 gap-1">
                        {perms.map((perm) => (
                          <label key={perm.id} className="flex items-center gap-1.5 cursor-pointer p-1 hover:bg-brand-600/10 rounded transition-colors">
                            <input type="checkbox" checked={formData.permissions.includes(perm.key)} onChange={() => togglePermission(perm.key)} className="w-3 h-3 rounded border-brand-600/50 bg-background dark:bg-background dark:bg-input accent-brand-500 dark:accent-[brand-400]" />
