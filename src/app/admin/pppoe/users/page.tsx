        ...
        <form onSubmit={handleSubmit}>
          <ModalBody className="space-y-4 max-h-[60vh] overflow-y-auto">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><ModalLabel required>{t('pppoe.password')}</ModalLabel>
                <div className="relative"><ModalInput type={showPassword ? 'text' : 'password'} value={formData.password} onChange={(e) => setFormData(prev => ({ ...prev, password: e.target.value }))} required className="pr-8" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-brand-400">{showPassword ? <EyeOff className="h-3 w-3" /> : <Eye className="h-3 w-3" />}</button>
                </div>
              </div>
            </div>
            <div><ModalLabel required>{t('pppoe.profile')}</ModalLabel><ModalSelect value={formData.profileId} onChange={(e) => setFormData(prev => ({ ...prev, profileId: e.target.value }))} required><option value="" className="dark:bg-input">{t('common.select')}</option>{profiles.map((p) => <option key={p.id} value={p.id} className="dark:bg-input">{p.name} - Rp {p.price.toLocaleString('id-ID')}</option>)}</ModalSelect></div>
            <div><ModalLabel>NAS ({t('common.optional')})</ModalLabel><ModalSelect value={formData.routerId} onChange={(e) => setFormData(prev => ({ ...prev, routerId: e.target.value }))}><option value="" className="dark:bg-input">{t('pppoe.global')}</option>{routers.map((r) => <option key={r.id} value={r.id} className="dark:bg-input">{r.name} ({r.ipAddress})</option>)}</ModalSelect></div>
            <div><ModalLabel>Area <span className="text-muted-foreground text-[10px]">({t('common.optional')})</span></ModalLabel><ModalSelect value={formData.areaId} onChange={(e) => setFormData(prev => ({ ...prev, areaId: e.target.value }))}><option value="" className="dark:bg-input">-- Pilih Area --</option>{areas.map((a) => <option key={a.id} value={a.id} className="dark:bg-input">{a.name}</option>)}</ModalSelect></div>
            <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <div className="flex items-center justify-between mb-1"><ModalLabel>{t('pppoe.gpsLocation')}</ModalLabel>
                <div className="flex gap-1">
                  <button type="button" onClick={() => setShowMapPicker(true)} className="inline-flex items-center px-2 py-0.5 text-[10px] bg-primary/10 text-primary border border-primary/50 rounded hover:bg-primary/20 dark:bg-brand-500/20 dark:text-brand-400 dark:border-brand-500/50 dark:hover:bg-brand-500/30"><Map className="h-2.5 w-2.5 mr-1" />{t('pppoe.openMap')}</button>
                  <button type="button" onClick={async () => { if (navigator.geolocation) { navigator.geolocation.getCurrentPosition((p) => { setFormData(prev => ({ ...prev, latitude: p.coords.latitude.toFixed(6), longitude: p.coords.longitude.toFixed(6) })); }, async () => { await showError(t('pppoe.gpsFailed')); }, { enableHighAccuracy: true, timeout: 10000 }); } }} className="inline-flex items-center px-2 py-0.5 text-[10px] bg-green-100 text-green-600 border border-green-300 rounded hover:bg-green-200 dark:bg-[success]/20 dark:text-[success] dark:border-[success]/50 dark:hover:bg-[success]/30"><MapPin className="h-2.5 w-2.5 mr-1" />{t('pppoe.autoGps')}</button>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className={`flex items-center p-2 border-2 rounded-lg cursor-pointer transition-all ${formData.subscriptionType === 'POSTPAID' ? 'border-primary bg-primary/10 dark:border-brand-500 dark:bg-brand-500/10 dark:shadow-[0_0_10px_rgba(70, 95, 255,0.3)]' : 'border-border hover:border-primary/50 dark:border-brand-600/30 dark:hover:border-brand-500/50'}`}>
                  <input type="radio" name="subscriptionType" value="POSTPAID" checked={formData.subscriptionType === 'POSTPAID'} onChange={(e) => setFormData(prev => ({ ...prev, subscriptionType: e.target.value as 'POSTPAID' }))} className="w-3 h-3 accent-primary dark:text-brand-400 border-border dark:border-brand-600/50 focus:ring-primary dark:focus:ring-[brand-400]" />
                ...
                <label className={`flex items-center p-2 border-2 rounded-lg cursor-pointer transition-all ${formData.subscriptionType === 'PREPAID' ? 'border-primary bg-primary/10 dark:border-brand-600 dark:bg-brand-600/10 dark:shadow-[0_0_10px_rgba(122, 90, 248,0.3)]' : 'border-border hover:border-primary/50 dark:border-brand-600/30 dark:hover:border-brand-600/50'}`}>
                  <input type="radio" name="subscriptionType" value="PREPAID" checked={formData.subscriptionType === 'PREPAID'} onChange={(e) => setFormData(prev => ({ ...prev, subscriptionType: e.target.value as 'PREPAID' }))} className="w-3 h-3 accent-primary dark:text-brand-600 border-border dark:border-brand-600/50 focus:ring-primary dark:focus:ring-[brand-600]" />
            ...
            {formData.subscriptionType === 'POSTPAID' && (
              <div><ModalLabel><Calendar className="w-3 h-3 inline mr-0.5" />{t('pppoe.billingDate')}</ModalLabel><ModalSelect value={formData.billingDay} onChange={(e) => setFormData(prev => ({ ...prev, billingDay: e.target.value }))}>{Array.from({ length: 31 }, (_, i) => i + 1).map(day => (<option key={day} value={day} className="dark:bg-input">{t('pppoe.dayOf')} {day}</option>))}</ModalSelect><p className="text-[10px] text-muted-foreground mt-1">{t('pppoe.monthlyDueDateDesc')}</p></div>
            )}
            ...
            {/* Dokumen Pelanggan */}
            <div className="border border-border dark:border-brand-600/30 rounded-lg p-3 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {formData.installationPhotos.map((photo, index) => (
                    <div key={index} className="relative">
                      ...
                      <img src={photo} alt={`Instalasi ${index + 1}`} className="w-full h-20 object-cover rounded border border-border dark:border-brand-500/20" />
    ...
    <div className="bg-background relative">
      ...
      <div className="relative z-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className="min-w-0">
              <h1 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-400 dark:via-white dark:to-accent-foreground dark:drop-shadow-[0_0_30px_rgba(70, 95, 255,0.5)]">{t('pppoe.title')}</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-4">
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)] hover:border-brand-600/50 transition-all">
            <div className="flex items-center justify-between">
              <div className="min-w-0"><p className="text-[10px] sm:text-xs text-brand-400 uppercase tracking-wide">{t('pppoe.registrationsThisMonth')}</p><p className="text-lg sm:text-2xl font-bold text-foreground mt-1">{registrationsThisMonth}</p></div>
              <UserPlus className="h-5 w-5 sm:h-8 sm:w-8 text-brand-400 drop-shadow-[0_0_15px_rgba(70, 95, 255,0.6)] flex-shrink-0" />
            </div>
          </div>
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)] hover:border-brand-600/50 transition-all">
            <div className="flex items-center justify-between">
              <div className="min-w-0"><p className="text-[10px] sm:text-xs text-brand-400 uppercase tracking-wide">{t('pppoe.renewalsThisMonth')}</p><p className="text-lg sm:text-2xl font-bold text-foreground mt-1">{renewalsThisMonth}</p></div>
              <RefreshCw className="h-5 w-5 sm:h-8 sm:w-8 text-green-400 drop-shadow-[0_0_15px_rgba(34,197,94,0.6)] flex-shrink-0" />
            </div>
          </div>
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)] hover:border-brand-600/50 transition-all">
            <div className="flex items-center justify-between">
              <div className="min-w-0"><p className="text-[10px] sm:text-xs text-brand-400 uppercase tracking-wide">{t('pppoe.isolatedExpired')}</p><p className="text-lg sm:text-2xl font-bold text-foreground mt-1">{isolatedExpired}</p></div>
              <Clock className="h-5 w-5 sm:h-8 sm:w-8 text-amber-400 drop-shadow-[0_0_15px_rgba(251,191,36,0.6)] flex-shrink-0" />
            </div>
          </div>
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)] hover:border-brand-600/50 transition-all">
            <div className="flex items-center justify-between">
              <div className="min-w-0"><p className="text-[10px] sm:text-xs text-brand-400 uppercase tracking-wide">{t('pppoe.blockedUsers')}</p><p className="text-lg sm:text-2xl font-bold text-foreground mt-1">{blockedUsers}</p></div>
              ...
              filteredUsers.map((user) => (
                <div key={user.id} className="p-3 space-y-2 active:bg-muted/50 transition-colors">
                  ...
                  <div className="flex items-center gap-1 pt-1 border-t border-border/50 flex-wrap">
                    ...
                    <button onClick={() => handleEdit(user)} className="compact-action p-1.5 text-brand-400 hover:bg-brand-500/10 rounded cursor-pointer flex items-center justify-center focus:outline-none" aria-label="Edit" title="Edit"><Pencil className="h-3.5 w-3.5 pointer-events-none" /></button>
                    ...
                    <tr key={user.id} className="hover:bg-muted/50">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <>
                              ...
                              {user.email && <p className="text-[10px] text-brand-400 truncate max-w-[140px]">{user.email}</p>}
                            </>
                            {user.area && (
                      ...
                      <td className="px-3 py-2 hidden xl:table-cell">
                        ...
                        {user.latitude && user.longitude && (
                          <a
                            ...
                            className="inline-flex items-center gap-0.5 text-[10px] text-brand-400 hover:text-brand-400/80 hover:underline cursor-pointer"
                      ...
                      <td className="px-3 py-2">
                        <div className="flex justify-end items-center gap-0.5">
                          ...
                          <button
                            onClick={() => handleEdit(user)}
                            className="compact-action p-1.5 text-brand-400 hover:bg-brand-500/10 rounded cursor-pointer focus:outline-none"
                            aria-label="Edit"
                            title="Edit"
                          >
        ...
        <SimpleModal isOpen={isImportDialogOpen} onClose={() => { setIsImportDialogOpen(false); setImportFile(null); setImportProfileId(''); setImportResult(null); }} size="md">
          ...
          <ModalBody className="space-y-4">
            ...
            <div>
              ...
              <input type="file" accept=".csv,.xlsx,.xls" onChange={(e) => setImportFile(e.target.files?.[0] || null)} className="w-full px-3 py-2 text-xs bg-background dark:bg-input border border-border dark:border-brand-600/40 rounded-lg text-foreground file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-primary/20 dark:file:bg-brand-600/30 file:text-foreground hover:file:bg-primary/30 dark:hover:file:bg-brand-600/50 focus:border-primary dark:focus:border-brand-500 focus:ring-1 focus:ring-primary/30 dark:focus:ring-[brand-400]/30 transition-all" />
        ...
        <SimpleModal isOpen={isSyncDialogOpen} onClose={() => { setIsSyncDialogOpen(false); setSyncPreview(null); setSyncResult(null); }} size="xl">
          <ModalHeader>
            <ModalTitle className="flex items-center gap-2"><RefreshCcw className="h-4 w-4 text-brand-400" />{t('pppoe.syncPppoeTitle')}</ModalTitle>
          ...
          <ModalBody className="space-y-4 max-h-[60vh] overflow-y-auto">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <ModalSelect value={syncRouterId} onChange={(e) => { setSyncRouterId(e.target.value); setSyncPreview(null); setSyncResult(null); }}>
                  <option value="" className="dark:bg-input">-- Pilih Router --</option>
                  {routers.map((r) => <option key={r.id} value={r.id} className="dark:bg-input">{r.name} ({r.ipAddress})</option>)}
                </ModalSelect>
              </div>
              <div>
                ...
                <ModalSelect value={syncProfileId} onChange={(e) => setSyncProfileId(e.target.value)}>
                  <option value="" className="dark:bg-input">{t('pppoe.selectProfile')}</option>
                  {profiles.map((p) => <option key={p.id} value={p.id} className="dark:bg-input">{p.name} - Rp {p.price.toLocaleString('id-ID')}</option>)}
                </ModalSelect>
              </div>
            </div>
            <button onClick={handleSyncPreview} disabled={!syncRouterId || syncLoading} className="w-full px-3 py-2 text-xs bg-primary text-primary-foreground hover:bg-primary/90 rounded-lg shadow-md disabled:opacity-50 disabled:shadow-none flex items-center justify-center gap-2 transition-all dark:bg-gradient-to-r dark:from-brand-400 dark:to-brand-600 dark:hover:from-brand-400/80 dark:hover:to-brand-600/80 dark:text-white dark:shadow-[0_0_15px_rgba(70, 95, 255,0.4)]">
            ...
            {syncPreview && (
              <div className="border border-border dark:border-brand-600/40 rounded-lg overflow-hidden bg-muted/30 dark:bg-input/50">
                <div className="px-3 py-2 bg-muted/50 dark:bg-brand-600/10 border-b border-border dark:border-brand-600/30 flex items-center justify-between">
                  <div className="text-xs">
                    <span className="font-medium text-foreground">{syncPreview.router?.name}</span>
                    <span className="text-muted-foreground ml-2">Total: {syncPreview.data?.total} | Baru: <span className="text-green-600 dark:text-[success] font-medium">{syncPreview.data?.new}</span> | Sudah ada: <span className="text-orange-500 dark:text-[#ff8c00]">{syncPreview.data?.existing}</span></span>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => toggleSyncSelectAll(true)} className="text-[10px] text-primary dark:text-brand-400 hover:underline">{t('pppoe.selectAllNew')}</button>
                ...
                <div className="max-h-60 overflow-y-auto">
                  <table className="w-full text-xs">
                    <thead className="bg-muted dark:bg-input sticky top-0">
                    ...
                    <tbody className="divide-y divide-border dark:divide-[brand-600]/20">
                      {syncPreview.data?.secrets?.map((secret: any) => (
                        <tr key={secret.username} className={`${secret.isNew ? 'bg-green-50 dark:bg-[success]/5' : 'bg-muted/50 dark:bg-brand-600/5'} ${secret.disabled ? 'opacity-50' : ''}`}>
                          <td className="px-2 py-1.5 text-center"><input type="checkbox" checked={syncSelectedUsers.has(secret.username)} onChange={() => toggleSyncSelectUser(secret.username)} disabled={!secret.isNew || secret.disabled} className="w-3 h-3 rounded accent-primary dark:accent-[brand-400]" /></td>
                          ...
                          <td className="px-2 py-1.5">
                            {secret.disabled ? (<span className="px-1.5 py-0.5 bg-muted text-muted-foreground rounded text-[9px]">{t('pppoe.disabledLabel')}</span>) : secret.isNew ? (<span className="px-1.5 py-0.5 bg-green-100 text-green-600 dark:bg-[success]/20 dark:text-[success] rounded text-[9px]">{t('pppoe.new')}</span>) : (<span className="px-1.5 py-0.5 bg-orange-100 text-orange-600 dark:bg-[#ff8c00]/20 dark:text-[#ff8c00] rounded text-[9px]">{t('pppoe.existing')}</span>)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                {syncSelectedUsers.size > 0 && (<div className="px-3 py-2 bg-primary/10 dark:bg-brand-500/10 border-t border-primary/30 dark:border-brand-500/30 text-xs text-primary dark:text-brand-400">? {syncSelectedUsers.size} {t('pppoe.usersSelectedToImport')}</div>)}
              </div>
            )}
            {syncResult && (
              <div className={`p-3 rounded-lg border ${syncResult.stats?.failed > 0 ? 'bg-orange-50 dark:bg-[#ff8c00]/10 border-orange-300 dark:border-[#ff8c00]/30' : 'bg-green-50 dark:bg-[success]/10 border-green-300 dark:border-[success]/30'}`}>
                <div className="text-xs space-y-1">
                  <div className="font-medium text-foreground">{syncResult.message}</div>
                  <div className="flex gap-4 text-[10px]">
                    <span className="text-green-600 dark:text-[success]">? Imported: {syncResult.stats?.imported}</span>
            ...
            <div className="p-3 bg-primary/10 dark:bg-brand-500/10 border border-primary/30 dark:border-brand-500/30 rounded-lg text-[10px] text-primary dark:text-brand-400">
              <p className="font-medium mb-1">?? {t('pppoe.infoTitle')}</p>
              <ul className="list-disc list-inside space-y-0.5 text-primary/80 dark:text-brand-400/80">
        ...
        <SimpleModal isOpen={isExtendModalOpen && !!selectedUserForExtend} onClose={() => { setIsExtendModalOpen(false); setSelectedUserForExtend(null); setSelectedProfileForExtend(''); }} size="md">
          ...
          {selectedUserForExtend && (
            <ModalBody className="space-y-4">
              <div className="bg-muted/30 dark:bg-input/50 rounded-lg p-3 space-y-2 text-xs border border-border dark:border-brand-600/30">
              ...
              <div>
                ...
                <ModalSelect value={selectedProfileForExtend} onChange={(e) => setSelectedProfileForExtend(e.target.value)}>
                  {profiles.map((p) => (<option key={p.id} value={p.id} className="dark:bg-input">{p.name} - Rp {p.price.toLocaleString('id-ID')}{p.id === selectedUserForExtend.profile.id ? ` ${t('pppoe.currentPackageLabel')}` : ''}</option>))}
                </ModalSelect>
              ...
              <div className="bg-primary/10 dark:bg-brand-500/10 border border-primary/30 dark:border-brand-500/30 rounded-lg p-3 text-xs"><p className="text-primary dark:text-brand-400">?? {t('pppoe.extendPaymentInfo')}</p></div>
            </ModalBody>
          )}
          <ModalFooter>
        ...
        <SimpleModal isOpen={isBroadcastDialogOpen} onClose={() => setIsBroadcastDialogOpen(false)} size="lg">
          <ModalHeader>
            <ModalTitle className="flex items-center gap-2"><Bell className="h-4 w-4 text-accent-foreground" />{t('pppoe.broadcastNotification')} {selectedUsers.size} User</ModalTitle>
          ...
          <ModalBody className="space-y-4 max-h-[60vh] overflow-y-auto">
              ...
              <>
                <div>
                  ...
                  <ModalSelect value={broadcastData.status} onChange={(e) => setBroadcastData({ ...broadcastData, status: e.target.value })}>
                    <option value="in_progress" className="dark:bg-input">?? {t('pppoe.outageInProgress')}</option>
                    <option value="resolved" className="dark:bg-input">? {t('pppoe.outageResolved')}</option>
                  </ModalSelect>
                </div>
                {broadcastData.status === 'in_progress' ? (
                  ...
                  <>
                    <div className="bg-[success]/10 border border-[success]/30 rounded-lg p-3"><p className="text-xs text-[success]">? {t('pppoe.repairCompletedInfo').replace('{count}', String(selectedUsers.size))}</p></div>
            ...
            {notificationType === 'payment' && (
              <>
                <div className="bg-[success]/10 border border-[success]/30 rounded-lg p-3"><p className="text-xs text-[success]">{t('pppoe.paymentReceiptInfo').replace('{count}', String(selectedUsers.size))}</p></div>
            ...
            <div>
              ...
              <div className="space-y-2 mt-2">
                <label className={`flex items-center gap-2 p-2 border-2 rounded-lg cursor-pointer transition-all ${broadcastData.notificationMethod === 'whatsapp' ? 'border-[#25D366] bg-[#25D366]/10 shadow-[0_0_10px_rgba(37,211,102,0.3)]' : 'border-brand-600/30 hover:border-[#25D366]/50'}`}>
                ...
                <label className={`flex items-center gap-2 p-2 border-2 rounded-lg cursor-pointer transition-all ${broadcastData.notificationMethod === 'email' ? 'border-brand-500 bg-brand-500/10 shadow-[0_0_10px_rgba(70, 95, 255,0.3)]' : 'border-brand-600/30 hover:border-brand-500/50'}`}>
                  <input type="radio" value="email" checked={broadcastData.notificationMethod === 'email'} onChange={(e) => setBroadcastData({ ...broadcastData, notificationMethod: e.target.value })} className="w-3.5 h-3.5 accent-[brand-400]" />
                  <span className="text-xs text-foreground">{t('pppoe.emailOnly')}</span>
                </label>
                <label className={`flex items-center gap-2 p-2 border-2 rounded-lg cursor-pointer transition-all ${broadcastData.notificationMethod === 'both' ? 'border-brand-600 bg-brand-600/10 shadow-[0_0_10px_rgba(122, 90, 248,0.3)]' : 'border-brand-600/30 hover:border-brand-600/50'}`}>
                  <input type="radio" value="both" checked={broadcastData.notificationMethod === 'both'} onChange={(e) => setBroadcastData({ ...broadcastData, notificationMethod: e.target.value })} className="w-3.5 h-3.5 accent-[brand-600]" />
