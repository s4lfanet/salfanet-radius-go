...
export default function AreasPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]"><MapPin className="w-12 h-12 text-brand-500 dark:text-brand-400 dark:drop-shadow-[0_0_20px_rgba(70, 95, 255,0.6)] relative z-10 animate-pulse" /></div>
    );
  }
    ...
    <div className="bg-background relative">
      ...
      <div className="relative z-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-400 dark:via-white dark:to-accent-foreground dark:drop-shadow-[0_0_30px_rgba(70, 95, 255,0.5)]">{t('pppoe.areasTitle')}</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
          <div className="bg-card/80 backdrop-blur-xl rounded-xl border-2 border-brand-600/30 p-2.5 sm:p-4 shadow-[0_0_20px_rgba(122, 90, 248,0.2)] hover:border-brand-600/50 transition-all">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs text-brand-400 uppercase tracking-wide">{t('pppoe.totalArea')}</p>
        ...
        <div className="block md:hidden space-y-3">
          {filteredAreas.length === 0 ? (
            <div className="bg-card/80 backdrop-blur-xl rounded-xl border border-brand-600/20 p-6 text-center text-muted-foreground text-xs">
              {areas.length === 0 ? t('pppoe.noArea') : t('pppoe.noMatchArea')}
            </div>
          ) : (
            filteredAreas.map((area) => (
              <div key={area.id} className="bg-card/80 backdrop-blur-xl rounded-xl border border-brand-600/20 p-3">
        ...
        <SimpleModal isOpen={isDialogOpen} onClose={() => { setIsDialogOpen(false); setEditingArea(null); resetForm(); }} size="md">
          ...
          <form onSubmit={handleSubmit}>
            <ModalBody className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="isActive" checked={formData.isActive} onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })} className="rounded border-brand-600/50 bg-background dark:bg-input accent-brand-500 dark:accent-[brand-400] w-4 h-4" />
