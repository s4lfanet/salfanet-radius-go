            ...
            <ModalBody className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={formData.isDefault} onChange={(e) => setFormData({ ...formData, isDefault: e.target.checked })} className="rounded border-brand-600/50 bg-background dark:bg-input accent-brand-500 dark:accent-[brand-400] w-3.5 h-3.5" />
                  <span className="text-xs text-foreground">{t('hotspot.setDefault')}</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={formData.isActive} onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })} className="rounded border-brand-600/50 bg-background dark:bg-input accent-brand-500 dark:accent-[brand-400] w-3.5 h-3.5" />
                  <span className="text-xs text-foreground">{t('common.active')}</span>
                </label>
              </div>
            </ModalBody>
            <ModalFooter className="justify-between">
              <button type="button" onClick={() => setShowPreview(true)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-brand-400 border border-brand-500/50 rounded-lg hover:bg-brand-500/10 transition-all">
