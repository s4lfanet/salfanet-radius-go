...
export default function PaymentGatewayPage() {
  ...
  const fetchConfigs = async () => {
    try {
      ...
      // Fetch QRIS Mandiri settings from company
      try {
        const qrisRes = await fetch('/api/company');
        if (qrisRes.ok) {
          const companyData = await qrisRes.json();
          setQrisForm({
  ...
  const saveQris = async () => {
    ...
    try {
      ...
      if (res.ok) {
        await showSuccess('QRIS Mandiri berhasil disimpan');
      } else {
