...
export default function PaymentPage() {
  ...
  const getStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      PAID: 'bg-[success]/20 text-[success] border border-[success]/40',
      PENDING: 'bg-[accent-foreground]/20 text-[accent-foreground] border border-[accent-foreground]/40',
  ...
  if (loading) return (
    <div className="min-h-screen bg-[secondary] relative overflow-hidden flex items-center justify-center">
      
      <div className="text-center relative z-10">
        <Loader2 className="w-10 h-10 animate-spin mx-auto text-[brand-400] drop-shadow-[0_0_20px_rgba(70, 95, 255,0.6)] mb-3" />
  ...
  if (error || !invoice) return (
    <div className="min-h-screen bg-[secondary] relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="relative z-10 bg-[secondary]/80 backdrop-blur-xl rounded-xl border-2 border-[destructive]/50 p-6 max-w-sm w-full text-center shadow-[0_0_50px_rgba(220, 38, 38,0.2)]">
        <AlertCircle className="w-12 h-12 text-[destructive] mx-auto mb-3 drop-shadow-[0_0_15px_rgba(220, 38, 38,0.5)]" />
  ...
  if (invoice.status === 'PAID') return (
    <div className="min-h-screen bg-[secondary] relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="relative z-10 bg-[secondary]/80 backdrop-blur-xl rounded-xl border-2 border-[success]/50 p-6 max-w-sm w-full text-center shadow-[0_0_50px_rgba(18, 183, 106,0.2)]">
        <div className="w-14 h-14 bg-[success]/20 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-[success]/50 shadow-[0_0_30px_rgba(18, 183, 106,0.3)]">
          <CheckCircle className="w-7 h-7 text-[success] drop-shadow-[0_0_10px_rgba(18, 183, 106,0.8)]" />
        </div>
        ...
        <div className="bg-[input]/50 rounded-xl p-4 text-left space-y-2">
          ...
          <div className="flex justify-between text-xs"><span className="text-[muted-foreground]/60">Jumlah</span><span className="font-bold text-[success]">{formatCurrency(invoice.amount)}</span></div>
    ...
    <div className="min-h-screen bg-[secondary] relative py-6 px-4">
      ...
      <div className="max-w-lg mx-auto space-y-4 relative z-10">
        ...
        <div className="text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[brand-600] to-[brand-400] rounded-full mb-3 shadow-[0_0_20px_rgba(122, 90, 248,0.4)]">
        ...
        {/* Invoice Card */}
        <div className="bg-[secondary]/80 backdrop-blur-xl rounded-xl border-2 border-[brand-600]/30 overflow-hidden shadow-[0_0_30px_rgba(122, 90, 248,0.15)]">
          <div className="bg-gradient-to-r from-[brand-600] to-[brand-400] px-4 py-3">
          ...
          <div className="p-4 space-y-4">
            {/* Invoice Number */}
            <div className="flex justify-between items-center pb-3 border-b border-[brand-600]/20">
              <span className="text-xs text-[muted-foreground]/60">Nomor Tagihan</span>
              <span className="font-mono font-bold text-sm text-[brand-400]">{invoice.invoiceNumber}</span>
            </div>

            {/* Customer Info */}
            <div>
              <p className="text-[10px] font-bold text-[brand-400] uppercase tracking-widest mb-2 flex items-center gap-2">
                <span className="w-6 h-[1px] bg-gradient-to-r from-[brand-400] to-transparent"></span>
                Informasi Pelanggan
              </p>
              <div className="bg-[input]/50 rounded-xl p-3 space-y-2.5">
                {/* Nama */}
                <div className="flex justify-between items-start text-xs gap-2">
                  <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><User className="w-3 h-3 text-[brand-600]" />Nama</span>
                  <span className="font-semibold text-white text-right">{invoice.user?.name || invoice.customerName}</span>
                </div>
                {/* Username */}
                {invoice.user?.username && (
                  <div className="flex justify-between items-start text-xs gap-2">
                    <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><Hash className="w-3 h-3 text-[brand-600]" />Username</span>
                    <span className="font-mono text-[brand-400] text-right">{invoice.user.username}</span>
                  </div>
                )}
                {/* Customer ID */}
                {invoice.user?.customerId && (
                  <div className="flex justify-between items-start text-xs gap-2">
                    <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><Hash className="w-3 h-3 text-[brand-400]" />ID Pelanggan</span>
                    <span className="font-mono text-white text-right">{invoice.user.customerId}</span>
                  </div>
                )}
                {/* Telepon */}
                <div className="flex justify-between items-start text-xs gap-2">
                  <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><Phone className="w-3 h-3 text-[brand-400]" />Telepon</span>
                  <span className="font-medium text-white text-right">{invoice.user?.phone || invoice.customerPhone}</span>
                </div>
                {/* Email */}
                {invoice.user?.email && (
                  <div className="flex justify-between items-start text-xs gap-2">
                    <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><Mail className="w-3 h-3 text-[accent-foreground]" />Email</span>
                ...
                {invoice.user?.address && (
                  <div className="flex justify-between items-start text-xs gap-2">
                    <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><MapPin className="w-3 h-3 text-[accent-foreground]" />Alamat</span>
                ...
                {invoice.user?.area?.name && (
                  <div className="flex justify-between items-start text-xs gap-2">
                    <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><Network className="w-3 h-3 text-[brand-600]" />Area</span>
                    <span className="font-medium text-white text-right">{invoice.user.area.name}</span>
                  </div>
                )}

                {/* Divider */}
                <div className="border-t border-[brand-600]/15 pt-2 space-y-2.5">
                  {/* Paket */}
                  {invoice.user?.profile && (
                    <div className="flex justify-between items-start text-xs gap-2">
                      <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><Package className="w-3 h-3 text-[accent-foreground]" />Paket</span>
                      <div className="text-right">
                        <p className="font-semibold text-white">{invoice.user.profile.name}</p>
                        {(invoice.user.profile.downloadSpeed > 0) && (
                          <p className="text-[10px] text-[brand-400]/70 flex items-center justify-end gap-1"><Zap className="w-2.5 h-2.5" />{invoice.user.profile.downloadSpeed}M / {invoice.user.profile.uploadSpeed}M</p>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between items-center text-xs gap-2">
                    <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><CreditCard className="w-3 h-3 text-[brand-400]" />Tipe</span>
                    <div className="flex items-center gap-1.5">
                      <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${
                        invoice.user?.subscriptionType === 'PREPAID'
                          ? 'bg-[accent-foreground]/20 text-[accent-foreground] border border-[accent-foreground]/30'
                  ...
                  {invoice.user?.router?.shortname && (
                    <div className="flex justify-between items-start text-xs gap-2">
                      <span className="text-[muted-foreground]/60 flex items-center gap-1.5 shrink-0"><Router className="w-3 h-3 text-[brand-600]" />Router</span>
            ...
            {/* Amount */}
            <div className="bg-gradient-to-br from-[brand-600]/20 to-[brand-400]/20 rounded-xl p-5 text-center border border-[brand-600]/30">
              <p className="text-[10px] text-[muted-foreground]/60 mb-1">Total Tagihan</p>
              <p className="text-3xl font-bold text-[brand-400] drop-shadow-[0_0_15px_rgba(70, 95, 255,0.5)]">{formatCurrency(invoice.amount)}</p>
            </div>

            {/* Dates */}
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-[input]/50 rounded-xl p-3">
                <p className="text-[10px] text-[muted-foreground]/60 mb-0.5 flex items-center gap-1"><Calendar className="w-3 h-3 text-[brand-600]" />Tanggal Terbit</p>
                <p className="text-xs font-medium text-white">{formatDate(invoice.createdAt)}</p>
              </div>
              <div className="bg-[input]/50 rounded-xl p-3">
                <p className="text-[10px] text-[muted-foreground]/60 mb-0.5 flex items-center gap-1"><Calendar className="w-3 h-3 text-[brand-400]" />Jatuh Tempo</p>
            ...
            {invoice.status === 'OVERDUE' && (
              <div className="bg-[destructive]/10 border border-[destructive]/30 rounded-xl p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <p className="text-xs font-bold text-[destructive]">Pembayaran Terlambat</p>
                    <p className="text-[10px] text-[destructive]/80 mt-0.5">Segera lakukan pembayaran.</p>
                  </div>
                </div>
              </div>
            )}
        ...
        {/* Payment Methods */}
        <div className="bg-[secondary]/80 backdrop-blur-xl rounded-xl border-2 border-[brand-600]/30 overflow-hidden shadow-[0_0_30px_rgba(122, 90, 248,0.15)]">
          <div className="px-4 py-3 border-b border-[brand-600]/20">
              ...
              <div className="space-y-2">
                {qrisOwn && (
                  <button
                    ...
                    disabled={processing}
                    className="w-full flex items-center justify-between p-4 bg-gradient-to-r from-[success]/40 to-[success]/40 border-2 border-[success]/50 rounded-xl hover:border-[success-foreground] hover:shadow-[0_0_20px_rgba(18, 183, 106,0.2)] transition-all disabled:opacity-50"
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-gradient-to-br from-[success] to-[success] rounded-xl flex items-center justify-center shadow-[0_0_15px_rgba(18, 183, 106,0.3)]">
                        <QrCode className="w-5 h-5 text-white" />
                      </div>
                      <div className="text-left">
                        <p className="text-xs font-bold text-white">QRIS Langsung</p>
                        <p className="text-[10px] text-[muted-foreground]/70">Tanpa biaya admin</p>
                      </div>
                    </div>
                    {processing ? (
                    ...
                    if (duitkuMethods.length > 0) {
                      return (
                        <div key={gateway.id} className="space-y-2">
                          <p className="text-[10px] font-bold text-[brand-400] uppercase tracking-widest px-1">{gateway.name}</p>
                          {duitkuMethods.map((method) => (
                            <button
                              key={method.code}
                              onClick={() => handlePayment('duitku', method.code)}
                              disabled={processing}
                              className="w-full flex items-center justify-between p-4 bg-[input]/50 border-2 border-[brand-600]/20 rounded-xl hover:border-[brand-400]/50 hover:bg-[input]/80 hover:shadow-[0_0_20px_rgba(70, 95, 255,0.1)] transition-all disabled:opacity-50"
                            >
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-gradient-to-br from-[brand-600] to-[brand-400] rounded-xl flex items-center justify-center shadow-[0_0_15px_rgba(122, 90, 248,0.3)]">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs font-bold bg-gradient-to-r from-[brand-400] to-[brand-600] bg-clip-text text-transparent">{company?.name || 'ISP Billing'}</p>
        </div>
      </div>

      {/* ========== QRIS OVERLAY ========== */}
      {qrisData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0, 0, 0,0.95)', backdropFilter: 'blur(12px)' }}>
          <div className="max-w-sm w-full relative">
            {/* Close Button */}
            <button onClick={closeQris} className="absolute -top-2 -right-2 z-10 w-8 h-8 flex items-center justify-center bg-[secondary] border-2 border-[brand-600]/40 rounded-full text-[muted-foreground]/60 hover:text-white hover:border-[accent-foreground] transition-all">
              <X className="w-4 h-4" />
            </button>

            {/* SUCCESS STATE */}
            {qrisStatus === 'paid' && (
              <div className="bg-[secondary]/90 backdrop-blur-xl rounded-xl border-2 border-[success]/50 p-8 text-center shadow-[0_0_60px_rgba(18, 183, 106,0.3)]">
                <div className="w-20 h-20 bg-[success]/20 rounded-full flex items-center justify-center mx-auto mb-4 border-2 border-[success]/50 shadow-[0_0_40px_rgba(18, 183, 106,0.4)] animate-bounce">
                  <CheckCircle className="w-10 h-10 text-[success] drop-shadow-[0_0_15px_rgba(18, 183, 106,0.8)]" />
                </div>
                <h2 className="text-xl font-bold text-[success] mb-2">Pembayaran Berhasil!</h2>
                <p className="text-sm text-[muted-foreground]/70">Terima kasih, pembayaran Anda telah dikonfirmasi.</p>
                <p className="text-xs text-[muted-foreground]/50 mt-3">Memuat ulang halaman...</p>
              </div>
            )}

            {/* EXPIRED STATE */}
            {qrisStatus === 'expired' && (
              <div className="bg-[secondary]/90 backdrop-blur-xl rounded-xl border-2 border-[destructive]/50 p-8 text-center shadow-[0_0_40px_rgba(220, 38, 38,0.2)]">
                <AlertCircle className="w-12 h-12 text-[destructive] mx-auto mb-3 drop-shadow-[0_0_15px_rgba(220, 38, 38,0.5)]" />
                <h2 className="text-lg font-bold text-white mb-2">QRIS Kadaluarsa</h2>
                <p className="text-xs text-[muted-foreground]/60 mb-4">Silakan buat pembayaran baru.</p>
                <button onClick={closeQris} className="px-6 py-2.5 bg-gradient-to-r from-[brand-600] to-[brand-400] text-white text-sm font-bold rounded-xl">
                  Tutup
                </button>
              </div>
            )}
            ...
            {qrisStatus === 'failed' && (
              <div className="bg-[secondary]/90 backdrop-blur-xl rounded-xl border-2 border-[destructive]/50 p-8 text-center shadow-[0_0_40px_rgba(220, 38, 38,0.2)]">
                ...
                <p className="text-xs text-[muted-foreground]/60 mb-4">Silakan coba lagi.</p>
                <button onClick={closeQris} className="px-6 py-2.5 bg-gradient-to-r from-[brand-600] to-[brand-400] text-white text-sm font-bold rounded-xl">
                  Tutup
                </button>
              </div>
            ...
            {qrisStatus === 'pending' && (
              <div className="bg-[secondary]/90 backdrop-blur-xl rounded-xl border-2 border-[brand-600]/40 overflow-hidden shadow-[0_0_50px_rgba(122, 90, 248,0.2)]">
                {/* Header */}
                <div className="bg-gradient-to-r from-[brand-600] to-[brand-400] px-4 py-3 flex items-center justify-between">
                ...
                <div className="p-5 space-y-4">
                  {/* Amount */}
                  <div className="text-center">
                    <p className="text-[10px] text-[muted-foreground]/50 uppercase tracking-wider">Total Pembayaran</p>
                    <p className="text-2xl font-bold text-[brand-400] drop-shadow-[0_0_10px_rgba(70, 95, 255,0.4)]">{formatCurrency(invoice?.amount || 0)}</p>
                  </div>

                  {/* QR Code */}
                  <div className="flex justify-center">
                    <div className="bg-white p-4 rounded-xl shadow-[0_0_30px_rgba(122, 90, 248,0.3)]">
                      <QRCodeSVG
                  ...
                  {/* Instructions */}
                  <div className="bg-[input]/60 rounded-xl p-3 space-y-2">
                    <p className="text-[10px] font-bold text-[brand-400] uppercase tracking-widest">Cara Bayar:</p>
                    <div className="space-y-1.5">
                      {['Buka aplikasi e-wallet (GoPay, OVO, DANA, ShopeePay, dll)', 'Pilih menu "Scan QR" atau "Bayar"', 'Scan kode QR di atas', 'Konfirmasi dan selesaikan pembayaran'].map((step, i) => (
                        <div key={i} className="flex items-start gap-2">
                          <span className="w-4 h-4 bg-[brand-600]/30 rounded-full flex items-center justify-center text-[8px] font-bold text-[muted-foreground] flex-shrink-0 mt-0.5">{i + 1}</span>
                          <p className="text-[11px] text-[muted-foreground]/70">{step}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {qrisData.isQrisOwn ? (
                    ...
                    <>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <p className="text-[10px] text-[muted-foreground]/50">Menunggu pembayaran... (auto-check setiap 5 detik)</p>
                      </div>
                      ...
                      {qrisData.paymentUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center justify-center gap-2 w-full py-2.5 bg-[input]/50 border border-[brand-600]/30 rounded-xl text-[11px] text-[muted-foreground]/60 hover:text-white hover:border-[brand-400]/50 transition-all"
                        >
